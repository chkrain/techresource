...
function initDownloadNotifications() {
  document.querySelectorAll(".document-item").forEach((link) => {
    link.addEventListener("click", function (e) {
      ...
      console.log(`Начинаетсяскачивание:${fileName}`);
      if (typeof ym !== "undefined") {
        ym(105767211, "reachGoal", "document_download", { filename: fileName });
      }
    });
  });
}
function initProjectsGallery() {
  const projectsData = {
    1: {
      ...
      description:
        "Разработка современной системы для мониторинга промышленного оборудования с использованием Grafana. Система позволяет отслеживать параметры оборудования в реальном времени, генерировать отчеты и настраивать уведомления.",
      details: [
        { icon: "", label: "Производительность", value: "Увеличена на 40%" },
        { icon: "", label: "Экономия", value: "до 25%" },
        { icon: "", label: "Срок разработки", value: "2 месяца" },
    ...
    2: {
      ...
      description:
        "Система управления линией покраски торцов фанеры с визуализацией технологического процесса и мониторингом параметров оборудования.",
      details: [
    ...
    3: {
      ...
      description:
        "Система управления процессом переработки трепела с автоматическим дозированием компонентов и контролем качества продукции.",
      details: [
        { icon: "", label: "Точность дозирования", value: "±0.5%" },
        { icon: "", label: "Производительность", value: "+30%" },
        { icon: "", label: "Автоматизация", value: "90%" },
        { icon: "", label: "Масштаб", value: "Промышленный" },
      ],
      features: [
    ...
    4: {
      ...
      description:
        "Система управления буссером с мониторингом параметров и системой аварийных остановок для обеспечения безопасности процесса.",
      details: [
        { icon: "", label: "Безопасность", value: "Уровень 4" },
    ...
    5: {
      ...
      description:
        "Система управления производственной линией гипса с контролем температуры, влажности и автоматическим регулированием процесса.",
      details: [
        { icon: "", label: "Точность контроля", value: "±1°C" },
    ...
    6: {
      ...
      description:
        "Система управления процессом производства синтетического каучука с контролем реакторов и параметров реакции.",
      details: [
    ...
    7: {
      ...
      description:
        "Система управления процессом обогащения песка с автоматическим контролем качества и оптимизацией процесса.",
      details: [
    ...
    8: {
      ...
      description:
        "Система управления технологической линией производства тука с автоматизацией процесса и системой отчетности.",
      details: [
    ...
    9: {
      ...
      description:
        "Система управления установкой депарафинизации и обезвоживания для нефтеперерабатывающей промышленности.",
      details: [
        { icon: "", label: "Эффективность", value: "+25%" },
        { icon: "", label: "Энергосбережение", value: "17%" },
        { icon: "", label: "Качество", value: "Соответствие ГОСТ" },
        { icon: "", label: "Безопасность", value: "ATEX зона 2" },
      ],
      features: [
    ...
    10: {
      ...
      description:
        "Система управления линией фасовки и упаковки трепела с контролем веса и автоматической маркировкой.",
      details: [
        { icon: "", label: "Точность фасовки", value: "±10г" },
    ...
    11: {
      ...
      description:
        "Разработка дашбордов Grafana для мониторинга процесса переработки трепела с аналитикой в реальном времени.",
      details: [
    ...
    12: {
      ...
      description:
        "Разработка универсальной системы на базе Grafana для различных отраслей промышленности с модульной архитектурой.",
      details: [
  ...
  const closeButtons = document.querySelectorAll(
    ".close-panel-btn, .close-bottom-btn",
  );
  function openProjectPanel(projectId) {
    ...
    if (detailsContainer) {
      detailsContainer.innerHTML = project.details
        .map(
          (detail) =>
            `<div class="detail-card"><div class="detail-icon">${detail.icon}</div><div class="detail-label">${detail.label}</div><div class="detail-value">${detail.value}</div></div>`,
        )
        .join("");
    }
    if (featuresContainer) {
      featuresContainer.innerHTML = project.features
        .map(
          (feature) =>
            `<div class="feature-item"><div class="feature-icon"></div><div class="feature-text">${feature}</div></div>`,
        )
        .join("");
    }
    if (tagsContainer) {
      tagsContainer.innerHTML = project.techTags
        .map((tag) => `<span class="panel-tech-tag">${tag}</span>`)
        .join("");
    }
    if (galleryContainer) {
      if (project.gallery && project.gallery.length > 0) {
        galleryContainer.innerHTML = `<h3 class="gallery-title">Дополнительныефотографии</h3><div class="photo-thumbnails">${project.gallery.map((img, index) => `<div class="thumbnail"data-img-src="${img}"><img src="${img}"alt="Фото проекта ${index + 1}"loading="lazy"><div class="thumbnail-overlay"><span class="zoom-icon">🔍</span></div></div>`).join("")}</div>`;
